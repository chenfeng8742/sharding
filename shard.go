...
func NewShard(id int64, db *pg.DB, prefix string) *Shard {
	shard := &Shard{
		id: id,
		DB: db,
	}
	...
	shard.prefix = prefix
	shard.fmter.SetParam(shard.prefix, types.F(shard.Name()))
	shard.fmter.SetParam(shard.prefix+"_id", shard.Id())
	return shard
}
...
func (shard *Shard) Name() string {
	return shard.prefix + strconv.FormatInt(shard.id, 10)
}
